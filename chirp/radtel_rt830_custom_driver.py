...
class RT830Radio(chirp_common.CloneModeRadio, chirp_common.ExperimentalRadio):
    ...
    def get_memory(self, number):
        ...
        txlimit = bool(rec[12] & 0x02)
        # CHIRP's standard Duplex=off means receive-only. Map it to the
        # RT-830 TX-inhibit bit when the stored TX frequency is simplex.
        # If TX Limit was manually enabled on a repeater/split channel, retain
        # the programmed duplex/offset instead of hiding it as "off".
        if txlimit and txraw == rxraw:
            mem.duplex = "off"
            mem.offset = 0
        elif txraw == rxraw:
            mem.duplex = ""
    ...
    def set_memory(self, mem):
        ...
        duplex_off = (mem.duplex == "off")
        if mem.duplex == "":
            txfreq = mem.freq
        elif duplex_off:
            # Store a normal simplex TX frequency but assert the radio's
            # per-channel TX-inhibit bit below.
            txfreq = mem.freq
        elif mem.duplex == "+":
        ...
        if duplex_off or experimental_widerx:
            txlimit = True
        if txlimit:
            rec[12] |= 0x02
